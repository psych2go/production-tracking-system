...
export async function getProcessDurations(filters: {
  ...
  const byBatch = new Map<number, typeof records>();
...
export async function getProductionTrend(filters: {
  ...
  const packagingStage = await prisma.processStage.findUnique({
  ...
  const grouped = new Map<string, { batchCount: number; totalQuantity: number }>();
  for (const r of records) {
...
export async function getGroupedStatistics(filters: {
  ...
  for (const b of batches) {
    const key = (b as Record<string, unknown>)[groupBy] as string | null ?? "未指定";
