...
export function parsePagination(
  ...
  opts: { pageDefault?: number; pageSizeDefault?: number; maxPageSize?: number } = {}
): { page: number; pageSize: number } {
  const { pageDefault = 1, pageSizeDefault = 50, maxPageSize = 100 } = opts;

  const rawPage = Math.floor(Number(query.page));
  const page = Number.isFinite(rawPage) && rawPage >= 1 ? rawPage : pageDefault;
